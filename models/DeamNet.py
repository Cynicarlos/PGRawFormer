...
class DeamNet(nn.Module):
    def __init__(self, Isreal, in_channels=4):
        super(DeamNet, self).__init__()
        if Isreal:
            self.transform_function = transform_function(in_channels, 64)
            self.inverse_transform_function = Inverse_transform_function(64, in_channels)
        else:
