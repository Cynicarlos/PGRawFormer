...
import sys
sys.path.append('E:\Deep Learning\experiments\RawDenoising')
...
class VRWKV4(nn.Module):
    def __init__(self, 
        inp_channels=4,  
        dim=48,
        num_blocks=[4,6,6,8], 
        num_refinement_blocks=4,
        align_gt_with_result=False):
        ...
        self.output = nn.Sequential(
            nn.Conv2d(int(dim*2**1), dim, kernel_size=3, stride=1, padding=1, bias=True),
            nn.GELU(),
            nn.Conv2d(dim, 3*inp_channels, 1),
            nn.PixelShuffle(int(math.sqrt(inp_channels))) 
        )
    ...
    def forward(self, inp_img):
        ...
        self.data_out = out_dec_level1

        return out_dec_level1 

'''
FLOPs: 24.611930112 G
Params: 4.801164 M
'''    
def cal_model_complexity():
    import time, thop
    x = torch.rand((1,4,224,224)).cuda()
    model = VRWKV4(
        inp_channels=4,
        dim=32,
        num_blocks=[2,2,2,2],
        num_refinement_blocks=2).cuda()
        
    #--------time-------------------
    since = time.time()
    y = model(x)
    print(f"time: {time.time()-since} s") 
