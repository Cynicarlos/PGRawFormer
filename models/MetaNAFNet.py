...
class MetaInjection(nn.Module):
    def __init__(self, in_channels, num_heads, num_meta_keys, meta_embedding_dims, bias=True):
    ...
    def forward(self, x, metainfo):
        ...
        metainfo = metainfo.unsqueeze(-1).unsqueeze(-1) #(b, c, h, w)
...
class MetaNAFNet(nn.Module):
    ...
    def __init__(self, img_channel=3, width=16, 
        ...
        for num in enc_blk_nums:
            ...
            self.enc_meta_inject.append(MetaInjection(in_channels=chan, num_heads=chan//width, num_meta_keys=num_meta_keys, meta_embedding_dims=meta_embedding_dims))
        ...
        self.middle_meta_inject = MetaInjection(in_channels=chan, num_heads=chan//width, num_meta_keys=num_meta_keys, meta_embedding_dims=meta_embedding_dims)
        ...
        for num in dec_blk_nums:
            ...
            self.dec_meta_inject.append(MetaInjection(in_channels=chan, num_heads=chan//width, num_meta_keys=num_meta_keys, meta_embedding_dims=meta_embedding_dims))

        self.padder_size = 2 ** len(self.encoders)

    def forward(self, inp, metainfoidx):
        metainfo = []
        ...
        for encoder, meta_inject, down in zip(self.encoders, self.enc_meta_inject, self.downs):
            x = encoder(x)
        ...
        for decoder, up, enc_skip, meta_inject in zip(self.decoders, self.ups, encs[::-1], self.dec_meta_inject):
            x = up(x)
            x = x + enc_skip
            x = decoder(x)
            x = meta_inject(x, metainfo)
...
class NAFNetLocal(Local_Base, MetaNAFNet):
...
'''
FLOPs: 64.028788928 G
Params: 17.06218 M
Time comsumed: 0.05239701271057129 s
'''
...
if __name__ == '__main__':
    model = MetaNAFNet(img_channel=4, width=32, 
                    num_meta_keys=5,
                    meta_table_size=[30,26,26,250,3],
                    meta_embedding_dims=384,
                    middle_blk_num=1,
