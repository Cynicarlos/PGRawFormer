...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, num_meta_keys, meta_embedding_dims, ffn_expansion_factor, bias, LayerNorm_type):
        ...
        self.meta_inject = MetaInjection(in_channels=dim, num_meta_keys=num_meta_keys, dim=meta_embedding_dims)
        self.ffn = FeedForward(dim, ffn_expansion_factor, bias)

    def forward(self, x, metainfo):
        x = x + self.attn(self.norm1(x))
        x = x + self.ffn(self.meta_inject(self.norm2(x), metainfo))
