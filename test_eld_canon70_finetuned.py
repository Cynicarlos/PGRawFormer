...
if __name__ == "__main__":
    ...
    with open('configs/ELD_CanonEOS70D.yaml', 'r') as file:
        config = yaml.safe_load(file)
    set_random_seed(config['manual_seed'])
    model_name, model = build_model(config['model'])
    model = model.cuda()
    
    checkpoint = torch.load('/data/models/Carlos/RAWDenoising/runs/ELD_CanonEOS70D/checkpoints/best_model.pth') #31
