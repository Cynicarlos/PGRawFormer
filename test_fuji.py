...
import os
import torch
from datasets.SIDDataset import SIDFujiDataset
...
def test(model, dataloader, ratio, merge_test=False, num_patch=2, with_metainfo=False, save_images=False, saved_path=None):
    ...
    tqdm_loader = tqdm(dataloader, desc=f"Evaluating SID_Fuji ratio {ratio}", leave=False)
    
    with open(f'results/SID/Fuji_Ratio_{ratio}.txt', 'w') as f:
        for idx, data in enumerate(tqdm_loader):
            ...
            if save_images:
                visualize(input_raw_path=os.path.join('/data/dataset/Carlos/SID/Fuji/Fuji', input_path),pred_raw=merged_pred[0],saved_path=saved_path)
...
if __name__ == "__main__":
    ...
    data_dir='/data/dataset/Carlos/SID/Fuji' #30
    ...
    model = model.cuda()
    checkpoint = torch.load('/data/model/Carlos/RAWDenoising/runs/FUJI/checkpoints/best_model.pth') #30
    ...
    for test_file, ratio in zip(test_files, ratios):
        dataset = SIDFujiDataset(data_dir=data_dir, image_list_file=test_file, split='test')
